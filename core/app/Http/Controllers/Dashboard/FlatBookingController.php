        ...
        DB::transaction(function () use ($request) {
            ...
            foreach ($request->project_id as $index => $projectId) {
                ...
                $flatDetails = BookedFlatInfo::create([
                    ...
                    'price_per_sqft' => $request->price_per_sqft[$index] ?? null,
                    ...
                    'gas_connection_fee' => $request->gas_amount[$index] ?? null,
                    ...
                    'parking_fee' => $request->parking_amount[$index] ?? null,
                    'is_utility_included' => $request->is_utility_included[$index] ?? 0,
                    'utility_payment_scheme' => $request->utility_payment_scheme[$index] ?: null,
                    'utility_fee' => $request->utility_amount[$index] ?? null,
                    'extras_amount' => $request->extras_amount[$index] ?? null,
            ...
            foreach ($request->project_id as $index => $projectId) {
                ...
                $pricePerSqft = $request->price_per_sqft[$index] ?? null;
    ...
    public function update(Request $request, $id)
    {
        ...
        DB::transaction(function () use ($request, $id) {
            ...
            $FlatBooking->update([
                ...
                'discount_amount_total' => $request->discount_amount_total ?? null,
                'total_price' => $request->total_price,
                'booking_amount' => $request->booking_amount,
                'downpayment_amount' => $request->downpayment_amount,
                'extras_total' => $request->extras_amount_total ?? null,
                'due_amount_total' => $request->due_amount ?? null,
                'total_emi_amount' => $request->emi_amount,
                'emi_count' => $request->emi_count,
