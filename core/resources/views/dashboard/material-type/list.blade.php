            ...
            <div class="box-header dker">
                <div class="row">
                    <div class="col-lg-8 col-sm-6">
                        <h3>List of Material Types</h3>
                        <small>
                            <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
                            <a>Material Types</a> /
                            <a>List of Material Types</a>
                        </small>
                    </div>
                            ...
                            <div class="col-sm-5" style="float: right !important;">
                                @if(@Auth::user()->permissionsGroup->add_status)
                                    <a class="btn btn-fw primary w-100" style="overflow: hidden"
                                       href="{{route('material-type.create')}}">
                        ...
                        <table class="table table-bordered m-a-0">
                            ...
                            @foreach($materialTypes as $materialType)
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">{{ $materialType->material_type }}</td>
                                    {{-- <td class="h6 nowrap">
                                        {!! Form::text('row_no_'.$materialType->id,$materialType->row_no, array('class' => 'form-control row_no light','autocomplete'=>'off')) !!}
                                        @if (@Auth::user()->permissionsGroup->edit_status)
                                            <a href="{{ route("categoriesEdit",["id"=>$materialType->id]) }}">
                                                @if($materialType->icon !="")
                                                    <i class="fa {!! $materialType->icon !!} "></i>
                                                @endif
                                                {{ $title }}
                                            </a>
                                        @else
                                    ...
                                    <td class="text-center">
                                        <div class="dropdown {{ (($x+1) >= count($materialTypes))?"dropup":"" }}">
                                            ...
                                            <div class="dropdown-menu pull-right">
                                                @if(@Auth::user()->permissionsGroup->edit_status)
                                                    <a class="dropdown-item"
                                                       href="{{ route('material-type.edit', ['id'=>$materialType->id]) }}"><i
                                                            class="material-icons">&#xe3c9;</i> {{ __('backend.edit') }}
                                                    </a>
                                                @endif
                                                @if(@Auth::user()->permissionsGroup->delete_status)
                                                    <a class="dropdown-item text-danger"
                                                       onclick="DeleteMaterial('{{ $materialType->id }}')"><i
...
@push("after-scripts")
    <script type="text/javascript">
        ...
        function DeleteMaterial(id) {
            $("#category_delete_btn").attr("href", '{{ route("material-type.destroy", ":id") }}'.replace(':id', id));
